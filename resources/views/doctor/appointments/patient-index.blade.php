    ...
    $(document).ready(function () {
        ...
        var calendar = $('#full_calendar_events').fullCalendar({
            ...
            select: function (calendar_date, allDay,start,end,cell) {
                ...
                if (moment().format('YYYY-MM-DD') === calendar_date.format('YYYY-MM-DD') || calendar_date.isAfter(moment())) {
                    ...
                    $.ajax({
                        ...
                        complete: response => {
                            ...
                            if ( resp ) {
                                if ( resp.status ) {
                                    make_modal('patient-book-appointment-modal', resp.data.view, true, 'modal-lg');
                                }
                            }
                        },
                        success: function(response) {
                            ...
                            $.each(response.appointments, function(index, el) {
                                ...
                                if(response.available_slot.includes(booked_slot)){
                                }
                ...
                return false;
                if (moment().format('YYYY-MM-DD') === appointment_date.format('YYYY-MM-DD') || appointment_date.isAfter(moment())) {
                }
                else { }
            },
        });
        ...
        if ( add_view_url ) {
            ...
            $.ajax({
                ...
                complete: function(response) {
                    ...
                    if ( resp ) {
                        if ( resp.status ) {
                            make_view_modal( 'view-appointment-details-modal theme-offcanvas-xl', resp.data.view, true );
                            if ( $(document).find('.all-patient-appointments-table').length ) {
                                var table = $(document).find('.all-patient-appointments-table').DataTable({
                                    processing: true,
                                    serverSide: true,
                                    aaSorting: [],
            ...
            $.ajax({
                ...
                complete: function(response) {
                    ...
                    if ( resp ) {
                        ...
                        if ( resp.status ) {
                            make_view_modal( 'todays-patient-appointment-details-modal theme-offcanvas-xl', resp.data.view, true );
                            if ( $(document).find('.todays-patient-appointments-table').length ) {
                                var table = $(document).find('.todays-patient-appointments-table').DataTable({
                                    processing: true,
                                    serverSide: true,
                                    aaSorting: [],
            ...
            $.ajax({
                ...
                complete: function(response) {
                    ...
                    if ( resp ) {
                        console.log('resp', resp)
                        if ( resp.status ) {
                            make_view_modal( 'upcoming-patient-appointment-details-modal theme-offcanvas-xl', resp.data.view, true );
                            if ( $(document).find('.upcoming-patient-appointments-table').length ) {
                                var table = $(document).find('.upcoming-patient-appointments-table').DataTable({
                                    processing: true,
                                    serverSide: true,
                                    aaSorting: [],
            ...
            $.ajax({
                ...
                complete: function(response) {
                    ...
                    if ( resp ) {
                        console.log('resp', resp)
                        if ( resp.status ) {
                            make_view_modal( 'past-patient-appointment-details-modal make_view_modal theme-offcanvas-xl', resp.data.view, true );
                            if ( $(document).find('.past-patient-appointments-table').length ) {
                                var table = $(document).find('.past-patient-appointments-table').DataTable({
                                    processing: true,
                                    serverSide: true,
                                    aaSorting: [],
        ...
        if (response['isConfirmed']) {
             $.ajax({
                ...
                success: function (response) {
                    ...
                    {
                        reject_notification();
                        $(document).find('#all-patient-appointments-table').DataTable().ajax.reload();
                        $(document).find('#todays-patient-appointments-table').DataTable().ajax.reload();
                        $(document).find('#upcoming-patient-appointments-table').DataTable().ajax.reload();
                        $(document).find('#past-patient-appointments-table').DataTable().ajax.reload();
                    }                    
                },
                error: function (response) {
                   error_notification();
